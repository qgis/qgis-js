...
To not confuse any downstream bundler, the runtime is [dynamically loaded](../packages/qgis-js/src/loader.ts) in a way that it will not be processed: The dynamic `import()` of the runtime is marked with the `@vite-ignore` and `webpackIgnore` magic comments, so that bundlers leave it alone and it is resolved at runtime. Therefore **it is up to the end user to include the runtime files in the final build**.
